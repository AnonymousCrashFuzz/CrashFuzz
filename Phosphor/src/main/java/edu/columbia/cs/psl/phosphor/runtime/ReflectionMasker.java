...
public class ReflectionMasker {
    ...
    private static MethodInvoke fixAllArgs(Constructor c, Taint cTaint, LazyReferenceArrayObjTags in, Taint argstaint, TaintedReferenceWithObjTag prealloc, boolean implicitTracking,
        ...
        if(c == null) {
            ...
            fillInParams(c.getClass().getName(), "<init>", ret.a, in, c.getParameterTypes());
    ...
    public static MethodInvoke fixAllArgs(Method m, Taint mTaint, Object owner, Taint ownerTaint, LazyReferenceArrayObjTags in, Taint inTaint, TaintedReferenceWithObjTag prealloc, Object[] unused, Object alsoUnused) {
        ...
        //crashfuzz: start
        // if(m.getName().contains("submitJob")) {
        //     System.out.println("Reflection GY: "+m.getName()+" "+(in.taints==null?true:false));
        //     System.out.print("Reflection GY: "+m.getName()+" paratypes: ");
        //     for(Class t:ret.m.getParameterTypes()) {
        //         System.out.print(t.getSimpleName()+" ");
        //     }
        //     System.out.println();
        //     if(in.taints != null) {
        //         for(Taint t:in.taints) {
        //             System.out.println("Reflection GY "+m.getName()+", passed taint: "+t);
        //         }
        //     }
        //     System.out.println();
        // }
        ...
        int j = fillInParams(m.getClass().getName(), m.getName(), ret.a, in, ret.m.getParameterTypes());
     ...
     */
    //private static int fillInParams(LazyReferenceArrayObjTags targetArgs, LazyReferenceArrayObjTags providedArgs, Class<?>[] paramTypes) {//crashfuzz: commented out
    private static int fillInParams(String cname, String mname, LazyReferenceArrayObjTags targetArgs, LazyReferenceArrayObjTags providedArgs, Class<?>[] paramTypes) {
        //crashfuzz
        //  String desc = "(";
        //  int last = paramTypes.length-1;
        //  if(paramTypes.length > 0) {
        //      //TODO: The identified descriptor could be inaccurate. Fix it in the future.
        //      for(int i = 0; i< last; i++) {
        //          desc += "L"+paramTypes[i].getName().replace(".", "/")+";";
        //      }
        //      desc += ")"+"L"+paramTypes[last].getName().replace(".", "/")+";";
        //  } else {
        //      desc += ")";
        //  }
        //crashfuzz: end
        ...
        if(providedArgs != null) {
            ...
            for(Object providedArg : providedArgs.val) {
                ...
                if(providedArgs.taints == null) {
                    targetArgs.val[targetParamIndex++] = Taint.emptyTaint(); //crashfuzz commented out
                    //crashfuzz: start
                    // targetArgs.val[targetParamIndex] = FAVTaint.newFAVTaint(cname.replace(".", "/"),
                    //         mname, desc, FAVTaintType.REFLECTION.toString(), FAVTagType.APP.toString(),
                    //         "param"+(targetParamIndex-1)+":"+paramTypes[targetParamIndex-1].getName().replace(".", "/"));
                    // targetParamIndex++;
                    //crashfuzz: end
                } else {
            ...
            if(chars.length == SET_TAG_METHOD_LEN) {
                ...
                for(int i = 3; i < SET_TAG_METHOD_LEN; i++) {
                    if(chars[i] != SET_TAG_METHOD_CHARS[i]) {
                        match = false;
                        break;
                        //crashfuzz: here mask getPHOSPHOR_TAG and setPHOSPHOR_TAG
                    }
                }
            }
