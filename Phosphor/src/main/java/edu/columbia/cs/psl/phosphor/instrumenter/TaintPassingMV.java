...
// public class TaintPassingMV extends RubbyPassingMV {
    ...
    public void visitCode() {
        ...
        //crashfuzz: set FAV_RECORD_TAG and FAV_MSGWRAPPER_TAG to true
    ...
    public void visitVarInsn(int opcode, int var) {
        ...
        switch(opcode) {
            ...
            case Opcodes.ASTORE:
                ...
                /*
                //crashfuzz: record the astore operation
                if(!Configuration.FOR_JAVA){
                    System.out.println("instrument ASTORE");
                    super.visitInsn(Opcodes.DUP);
                    super.visitMethodInsn(INVOKESTATIC, "java/crashfuzz/record/instrumentation/RecordTaint", "recordTaint",  "("+Configuration.TAINT_TAG_DESC+")V", false);
                }
                //end crashfuzz
                */
        ...
        switch(opcode) {
            case Opcodes.GETSTATIC:
                if(owner.equals("org/apache/commons/math3/util/FastMathLiteralArrays") && name.equals("LN_MANT")) {
                    //crashfuzz: modified it to skip a hadoop too large multi-dimession array bug
        ...
        switch(opcode) {
            ...
            case Opcodes.NEW:
                ...
                */
                //crashfuzz end
                //crashfuzz: comment
                controlFlowPolicy.generateEmptyTaint();  //crashfuzz: new taint tag for a new instance
                //crashfuzz: comment end
                break;
        ...
        if (bsmArgs != null) {
            if (bsm.getName().equals("metafactory") || bsm.getName().equals("altMetafactory")) {
                ...
                for(int k = 0; k < bsmArgs.length; k++) {
                    ...
                    } else if (o instanceof Type) {//crashfuzz: rubby methods can use here!!!!!!
                        Type t = (Type) o;
    ...
    public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean isInterface) {
        ...
        //crashfuzz: for SocketChannelImpl
        if(Instrumenter.isSocketChannelWrite(this.owner, this.name, this.descriptor) && owner.startsWith("sun/nio/ch/IOUtil") && name.equals("write"+TaintUtils.METHOD_SUFFIX)) {
            ...
            */
            super.visitMethodInsn(opcode, owner, name, newDesc, isInterface);
        } else if (Instrumenter.isSocketChannelRead(this.owner, this.name, this.descriptor) && owner.startsWith("sun/nio/ch/IOUtil") && name.equals("read"+TaintUtils.METHOD_SUFFIX)) {
            ...
            */
            super.visitMethodInsn(opcode, owner, name, newDesc, isInterface);
        } else if (Instrumenter.isSocketChannelWriteArray(this.owner, this.name,
          ...
          //SocketChannelImpl write ByteBuffer[]
            ...
            */
            //crashfuzz: debug important point
            // super.visitLdcInsn("!!!!!!!!!!crashfuzz goint to WRITE bytebuffer array! This may introduce faults!!!!!!!!!!!!!!!");
            // FAV_PRINT_STRING.delegateVisit(mv);
            super.visitMethodInsn(opcode, owner, name, newDesc, isInterface);
        } else if (Instrumenter.isSocketChannelReadArray(this.owner, this.name,
                this.descriptor) && owner.startsWith("sun/nio/ch/IOUtil") &&
                name.equals("read"+TaintUtils.METHOD_SUFFIX)) {
            super.visitLdcInsn("!!!!!!!!!!crashfuzz goint to READ bytebuffer array! This may introduce faults!!!!!!!!!!!!!!!");
            FAV_PRINT_STRING.delegateVisit(mv);
            super.visitMethodInsn(opcode, owner, name, newDesc, isInterface);
        } else {
        	super.visitMethodInsn(opcode, owner, name, newDesc, isInterface);
    ...
    public void visitInsn(int opcode) {
        if(isLambda && OpcodesUtil.isReturnOpcode(opcode)) {
        	...
        	// 	super.visitMethodInsn(Opcodes.INVOKESTATIC, "java/crashfuzz/instrumenter/hbase/HBaseTrackingMV",
        	// 			"print", "(Ljava/lang/String;)V", false);
            // }
            //crashfuzz: gy try to fix a phosphor error
            ...
            //++
            //crashfuzz: gy fix phosphor error end
    ...
    private void visitReturn(int opcode) {
        ...
        String valDesc = opcode == ARETURN ? "Ljava/lang/Object;" : originalMethodReturnType.getDescriptor();
        if(this.owner.equals("org/apache/hadoop/mapred/TaskRunner") && this.name.startsWith("getVMArgs")){
            //crashfuzz: use original java for map tasks and reduce tasks in hadoop-1.2.0
            //discard this fix, since it caused by nonstandard configuration file.
            //super.visitMethodInsn(INVOKESTATIC, "java/crashfuzz/record/instrumentation/RecordTaint", "transJavaForMappedTasks", "(Ljava/util/Vector;)Ljava/util/Vector;", false);
        }
        super.visitFieldInsn(PUTFIELD, newReturnType.getInternalName(), "val", valDesc);
        super.visitVarInsn(ALOAD, retIdx);
        Configuration.taintTagFactory.stackOp(opcode, mv, lvs, this);
