...
public abstract class MultiDTaintedArrayWithObjTag {
    ...
    public static Type getTypeForMultiDArray(final Type originalElementType) {
        if(originalElementType.getSort() != Type.ARRAY) {
            throw new IllegalArgumentException("passed " + originalElementType);
        }
        int dimensions = originalElementType.getDimensions();
        String brackets = "";
        for(int i = 1; i < dimensions; i++) {
            brackets += "[";
        }
        return Type.getType(brackets + getDescriptorForComponentType(originalElementType.getElementType().getSort()));
    }
