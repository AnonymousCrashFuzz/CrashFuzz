...
public class WaitToExec { //for docker
    ...
    public static void checkCrashEvent(String path, String contentID) {
        ...
        List<String> callstack = RecordTaint.getCallStack(thread);
    ...
    public static void handleCrashPoint(long procID, String nodeIP, FAVEntry entry, List<String> callstack, String path) {
    	if(CurrentFaultSequence.faultSeq.curFault == -1 || CurrentFaultSequence.faultSeq.curFault >= CurrentFaultSequence.faultSeq.seq.size()) {
    		//no faults to inject
...
//    				+", "+CurrentFaultSequence.faultSeq.seq.size());
    	...
    	if(ioID != null) {
    		Stat.log(nodeIP+":"+procID+" System WaitToExec prepare to get permission to crash "+ Configuration.CONTROLLER_SOCKET
...
//                objOut.flush();
                ...
                if(fuzzCommand.equals(TriggerEvent.CONTI.toString())
                	...
                	Stat.log(nodeIP+":"+procID+" System WaitToExec received keep exec command!"+fuzzCommand+suffix);
                    return;
                } else if (fuzzCommand.equals(TriggerEvent.CRASH.toString())) {
                	Stat.log(nodeIP+":"+procID+" System WaitToExec received crash command!"+fuzzCommand+suffix);
                    //crashCurNode();
                    while(true) {
                         try {
                            Thread.sleep(500);
                	...
                	Stat.log(nodeIP+":"+procID+" System WaitToExec received abnormal message: "+fuzzCommand+suffix);
                }
            } catch(Exception e) {
            	Stat.log(nodeIP+":"+procID+" [ERROR] System WaitToExec got exception:"+e.getMessage());
                e.printStackTrace();
            }
            Stat.log(nodeIP+":"+procID+" [ERROR] System WaitToExec failed to trigger a current crash fault!!!"+fuzzCommand+", "+callstack);
    	} else {
